...
class LeftWallFollower:
    def __init__(self):
        ...
        self.voice_command_sub = rospy.Subscriber('voice_commands', String, self.voice_cb)
    ...
    def follow_left_wall(self):
        ...
        if self.running_state == False:
            print("Stop Command Received, Stop the Robot")
            twist.linear.x = 0
            twist.angular.z = 0
        else:
            print("The running state is True, Keep Running")
            # BANG BANG CONTROL
            if obstacle:
